...
class LLMClientOpenAI:
    def __init__(self, model: str = "gpt-4o-mini", temperature: float = 0.2, max_tokens: int = 250):
...
class LLMClientClaude:
    def __init__(self, model: str = "claude-sonnet-4-20250514", temperature: float = 0.2, max_tokens: int = 250):
        ...
        if model is None:
            self.model = os.getenv("MODEL_NAME", "claude-sonnet-4-20250514")  # Add fallback
        else:
            self.model = model
...
class MCPToolExecutor:
    def __init__(self, command: str = "python", args: List[str] = None):
        if args is None:
            args = ["/home/ts75080/Desktop/MCP_research_paper/mcp/paper_code/server.py"]
        ...
        async with stdio_client(self.server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                try:
                    ...
                    print(f'BMI for weight {args.get("weight_kg")}kg and height {args.get("height_m")}m is {result.content[0].text}')
